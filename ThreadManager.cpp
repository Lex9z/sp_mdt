...
//---------------------------------------------------------------------------
void ThreadManager::Terminate(int index)
{
    EnterCriticalSection(&_threads_cs);
...
//---------------------------------------------------------------------------
void ThreadManager::Mark_finish(int index)
{
   EnterCriticalSection(&_threads_cs);
...
//---------------------------------------------------------------------------
bool ThreadManager::Is_running(int index)
{
...
//---------------------------------------------------------------------------
void ThreadManager::Stop(int index)
{
   EnterCriticalSection(&_threads_cs);
...
//---------------------------------------------------------------------------
int * ThreadManager::Get_stop_flag_addr(int index)
{
   return &(_stop_flag[index]);
}

//---------------------------------------------------------------------------
bool ThreadManager::Is_stopped(int index)
{
...
//---------------------------------------------------------------------------
bool * ThreadManager::Get_enter_mpdl_flag_addr(int index)
{
   return &(_p_enter_mpdl_queue->_enter_mpdl_flag[index]);
}

//---------------------------------------------------------------------------
void ThreadManager::Apply_enter_mpdl_queue(int index)
{
    GC_TRACE("<ThreadManager(Idx: %d)> Apply Enter protected section", index);
    _p_enter_mpdl_queue->Apply(index);
}

//---------------------------------------------------------------------------
void ThreadManager::Enter_mpdl_finish(int index)
{
   GC_TRACE("<ThreadManager(Idx: %d)> Leave protected section", index);
...
void Enter_mpdl_queue::Apply(int index)
{
   ...
   GC_TRACE("<ThreadManager(Idx: %d)> Enter_mpdl_queue::Apply", index);
   ...
   try
   {
      if(!_enter_mpdl_queue.empty())
      {
          ...
          {
             GC_TRACE("<ThreadManager(Idx: %d)> queue size is %d", index, _enter_mpdl_queue.size());
             GC_TRACE("<ThreadManager(Idx: %d)> thread is running, may be some error occur", index);
      ...
      GC_TRACE("<ThreadManager(Idx: %d)> Add to queue success!", index);
...
void Enter_mpdl_queue::Finish(int index)
{
   ...
   try
   {
     if(!_enter_mpdl_queue.empty())
     {
        if(index == _enter_mpdl_queue.front())
        {
           GC_TRACE("<ThreadManager(Idx: %d)> Finish critical tourist.", index);
