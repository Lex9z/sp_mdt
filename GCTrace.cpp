...
{ 
	...
	test_handle = CreateFile(m_log_path.c_str(), GENERIC_WRITE,
...
void GCTrace::debug_on()
{
    ...
    {       /*
            ...
            m_h_log = CreateFile(m_log_path.c_str(), GENERIC_WRITE, FILE_SHARE_READ|FILE_SHARE_WRITE, NULL,
    ...
    base_tarce("\n=========================================================\n");
    base_tarce("| SP Multiport Flash Download Project Tarce On		 \n");
    base_tarce("|                                     			 \n");
...
void GCTrace::clear()
{
    ...
    m_h_log = CreateFile(m_log_path.c_str(), GENERIC_WRITE, FILE_SHARE_READ|FILE_SHARE_WRITE, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
...
void GCTrace::set_log_path(string  log_path)
{
    m_log_path = log_path;
}


void GCTrace::base_tarce(const char *pText, ...)
{
    va_list params;
    DWORD	wl;
...
void GCTrace::tarce(const char *pText, ...)
{
    va_list params;
    DWORD	wl;
    ...
    sprintf(trace_time, " [%02d-%02d-%02d %02d:%02d:%02d::%04d] ",
            system_time.wYear,
            system_time.wMonth,
            system_time.wDay,
            system_time.wHour,
...
void GCTrace::dump_binary_by_hex(const unsigned char * byte,  unsigned long len)
{
