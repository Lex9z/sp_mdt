...
/* Main function, block must be 64 bytes */
void MD5_Core(const unsigned char block[64])
{
    //if( !block ) return; //Note: I dont't check, but you must be sure of this
...
/*  Calculate MD5 of a buffer
    Note!!! ===>if "last_buffer" is false, in_buffer must always 64 bytes aligned<====
*/
int MD5_Buffer(const unsigned char *in_buffer, const unsigned int size, bool last_buffer)
{
    ...
    for(; index + 63 < size; index += 64)
    {
        MD5_Core(&in_buffer[index]);
    }

    /* If the last buffer, we should fill the buffer as the algorithm discript */
    if( last_buffer )
    {
        unsigned char temp_buffer[64];
        unsigned int len = size - index;

        memcpy(temp_buffer, &in_buffer[index], len);
        memcpy(&temp_buffer[len], PADDING, 64 - len);

        if(len >= 56) /* 64-8 bytes */
        {
            MD5_Core(temp_buffer);
            memset(temp_buffer, 0, 64);
        }

        *(unsigned int*)&temp_buffer[56] = count[0];
        *(unsigned int*)&temp_buffer[60] = count[1];
...
int MD5_File(const char *file_path)
{
    ...
    const int BUFFER_SIZE = 1024 * 1024; // must 64 bytes aligned
    unsigned int length = 0;
    unsigned char *buffer = (unsigned char*)malloc(BUFFER_SIZE);

    /* There has a bug, if the file size is just aligned BUFFER_SIZE, the MD5 is wrong, you can fix this */
    ...
    do
    {
        ...
        {
            MD5_Buffer(buffer, length, true);
            break;
        }
        else
        {
            MD5_Buffer(buffer, length, false);
        }
    } while( length );
